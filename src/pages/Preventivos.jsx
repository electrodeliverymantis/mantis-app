...
export default function Preventivos() {
  const { preventivos, cargando, crearPreventivo, marcarRealizado, eliminarPreventivo } = usePreventivos()
  ...
  const handleCrear = async () => {
    if (!titulo || !sector || !maquina || !proximaFecha) {
    ...
    try {
      ...
      setFrecuencia("Mensual"); setProximaFecha(""); setDescripcion("")
      setMostrarFormulario(false)
    } catch {
      setError("Error al crear el preventivo")
    } finally {
      setGuardando(false)
    }
  }

  const handleMarcarRealizado = async (preventivo) => {
    const nuevaFecha = calcularProximaFecha(preventivo.frecuencia)
    await marcarRealizado(preventivo.id, nuevaFecha)
  }

  const preventivosFiltrados = filtro === "todos"
    ? preventivos
    : preventivos.filter(p => p.estado === filtro)
  ...
  return (
    <div>
      ...
      {vencidos > 0 && (
        <div style={{
          background: "#fee2e2", border: "1px solid #ef444440",
          ...
          <div>
            <div style={{ fontWeight: 700, color: "#ef4444", fontSize: 14 }}>
              {vencidos} preventivo{vencidos > 1 ? 's' : ''} vencido{vencidos > 1 ? 's' : ''}
            </div>
            <div style={{ fontSize: 12, color: "#b91c1c" }}>
              Revisá y marcalos como realizados
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 16 }}>
          {preventivosFiltrados.map(p => {
            ...
            return (
              <div key={p.id} style={{
                ...
                border: p.estado === 'vencido' ? "2px solid #ef444440" : "1px solid #e2e8f0"
              }}>
                ...
                <div style={{ fontSize: 12, color: "#64748b", marginBottom: 12 }}>
                  📅 Próxima fecha: <strong>{p.proxima_fecha}</strong>
                </div>

                {isMant && (
                  <div style={{ display: "flex", gap: 8 }}>
                    <button onClick={() => handleMarcarRealizado(p)} style={{
                      flex: 1, padding: "8px", borderRadius: 8,
                      border: "1px solid #10b98140", background: "#d1fae5",
                      color: "#065f46", cursor: "pointer", fontSize: 12,
                      fontWeight: 600, fontFamily: "inherit"
                    }}>✅ Marcar realizado</button>
                    <button onClick={() => eliminarPreventivo(p.id)} style={{
          ...
          {preventivosFiltrados.length === 0 && (
            <div style={{ gridColumn: "1/-1", textAlign: "center", padding: 60, color: "#94a3b8", fontSize: 14 }}>
              No hay preventivos con este filtro
            </div>
          )}
        </div>
      )}
            ...
            <div style={{ padding: 24 }}>
              <div style={{ marginBottom: 14 }}>
                ...
                <input type="text" value={titulo} onChange={e => setTitulo(e.target.value)}
                  placeholder="Ej: Cambio de aceite - Torno CNC-01" style={inputStyle} />
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
                ...
                <div>
                  ...
                  <input type="date" value={proximaFecha} onChange={e => setProximaFecha(e.target.value)} style={inputStyle} />
                </div>
              </div>
              <div style={{ marginBottom: 14 }}>
                <label style={labelStyle}>Descripción de tareas</label>
                <textarea value={descripcion} onChange={e => setDescripcion(e.target.value)}
                  rows={3} placeholder="Describí las tareas a realizar..."
              ...
              <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
                ...
                <button onClick={handleCrear} disabled={guardando} style={{
                  ...
                  {guardando ? "Guardando..." : "Guardar Preventivo"}
                </button>
              </div>
            </div>
          </div>
